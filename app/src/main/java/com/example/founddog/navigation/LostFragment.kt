    ...
    inner class RecyclerViewAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
        ...
        override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
           ...
           var img: ImageView = viewholder.findViewById(R.id.img_recycle)
            ...
            val drawable: Drawable? = ResourcesCompat.getDrawable(resources, R.drawable.placeholder, null)

            Glide.with(holder.itemView.context)
                .load(postDTO[position].imgUrl)
                .thumbnail(0.1f)
                .placeholder(drawable)
                .into(img)
            viewholder.title_recycle.text = postDTO[position].title
